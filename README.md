...
A tiny Caddy container that permanently redirects all traffic to `https://www.getcolors.ai`, preserving the request path and query string.
...
1. `arm64` and `amd64` images build natively and in parallel on their own runners — no QEMU — and push per-architecture tags to `ghcr.io/getcolors/colors-redirect`.
2. A manifest job combines them into multi-arch `:latest` and `:sha-<short-sha>` tags.
3. A deploy job connects over SSH and runs `sudo once update getcolors.ai` on the server.

The deploy step needs the `SSH_PRIVATE_KEY`, `SERVER_IP`, and `SERVER_USER` repository secrets.
